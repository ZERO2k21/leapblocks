...
let _BOARD_NAME_BY_ID: Record<string, string> | undefined

export function getBoardNameById(): Record<string, string> {
  if (!_BOARD_NAME_BY_ID) {
    _BOARD_NAME_BY_ID = (getBoards() as BoardEntry[]).reduce<Record<string, string>>((acc, b) => {
      acc[b.id] = b.name
      return acc
    }, {})
  }
...
export const BOARD_UPLOAD_CONFIG: Record<string, BoardConfig> = {
  arduino_uno: {
...
export const getBoardConfig = (boardId: string): BoardConfig =>
  BOARD_UPLOAD_CONFIG[boardId] || BOARD_UPLOAD_CONFIG.arduino_uno
...
export const isBoardUploadFile = (fileName: string, boardEntryFile: string): boolean => {
...
export const sortUploadFiles = (fileNames: string[], preferredFile: string): string[] =>
...
export const createUploadFiles = (boardId: string): Record<string, string> => {
...
export const formatPortLabel = (port: PortInfo | null | undefined): string => {
  if (!port) return ""
...
export const buildMicroPythonTemplate = (boardName: string): string =>
  `# LeapBlocks Python Upload Mode
...
export const buildBoardTemplate = (boardId: string): string => {
  if (boardId === "esp32") {
...
export const getLibraryBaseName = (rawName: string = ""): string => {
...
export const getLibraryClassName = (baseName: string): string => {
...
export const getUniqueLibraryBaseName = (rawName: string, existingFiles?: Record<string, unknown>): string => {
...
export const buildLibraryHeaderTemplate = (rawName: string): LibraryFile => {
  const baseName = getLibraryBaseName(rawName)
...
export const buildLibraryCppTemplate = (rawName: string): LibraryFile => {
  const baseName = getLibraryBaseName(rawName)
  const className = getLibraryClassName(baseName)
...
export const normalizeCppInclude = (rawValue: string = ""): string => {
...
export const insertIncludeLineIntoSource = (sourceCode: string = "", includeLine: string): string => {
