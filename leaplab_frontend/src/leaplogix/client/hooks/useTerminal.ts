...
export function useTerminal() {
  const [terminalOutput, setTerminalOutput] = useState<LogEntry[]>([
    ...
    { text: "", type: "info", ts: new Date() },
    {
      text: !(window as any).electronAPI?.isElectron
  ...
  const addLog = useCallback((text: string, type: string = "log") => {
  ...
  const handleShellSubmit = useCallback(() => {
    ...
    if ((window as any).electronAPI?.isElectron) {
      ;(window as any).electronAPI.pythonShellRun(cmd)
    }

    setShellInput("")
  }, [shellInput, addLog])

  const handleShellKey = useCallback((e: React.KeyboardEvent) => {
