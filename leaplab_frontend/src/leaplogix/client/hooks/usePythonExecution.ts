...
interface UsePythonExecutionProps {
...
export function usePythonExecution({ projectFiles, setProjectFiles, addLog }: UsePythonExecutionProps) {
  ...
  const skulptRef = useRef<SkulptEngine | null>(null)
  const runStopRequestedRef = useRef(false)

  const initSkulpt = useCallback((actions: any) => {
  ...
  const handleRun = useCallback(async (activeFile: string, onResetStage?: () => void) => {
    if (isRunning) return
    ...
    try {
      ...
      if ((window as any).electronAPI?.isElectron) {
        if ((window as any).electronAPI.pythonCheck) {
          const pythonCheck = await (window as any).electronAPI.pythonCheck()
        ...
        setTerminalInputValue("")
        setTimeout(() => terminalInputRef.current?.focus(), 80)
        await (window as any).electronAPI.pythonRun(code, projectFiles)
      } else {
        if (!skulptRef.current) {
      ...
      const errorMsg = typeof e === 'string' ? e : (e as Error)?.message || (e as any)?.toString?.() || JSON.stringify(e) || "Unknown error"
      addLog(`────────────────────────────────────────`, "error")
      ...
      if ((window as any).electronAPI?.isElectron) {
        setIsRunning(false)
        try { ;(window as any).electronAPI.pythonStop() } catch (_) { /* noop */ }
      }
    } finally {
      if (!(window as any).electronAPI?.isElectron) {
        setIsRunning(false)
  ...
  const handleStop = useCallback(() => {
    ...
    if ((window as any).electronAPI?.isElectron) {
      ;(window as any).electronAPI.pythonStop()
    } else {
  ...
  const handleTerminalInputSubmit = useCallback(() => {
    ...
    if (inputResolverRef.current) {
      ...
      addLog(val, "input")
      ;(window as any).electronAPI.pythonSendInput(val)
      setTerminalInputValue("")
    }
  }, [terminalInputValue, addLog])

  const handleTerminalInputKey = useCallback((e: React.KeyboardEvent) => {
