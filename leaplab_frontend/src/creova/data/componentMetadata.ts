import { PALETTE_ENHANCED } from './paletteComponents_Enhanced';
import { GENERATED_COMPONENT_METADATA } from './componentMetadata.generated';
...
interface AnyComponentDef {
  methods: string[];
  properties: string[];
}

export const COMPONENT_METADATA: Record<string, ComponentDef> = {
  'Screen': {
    events: [
...
export const ANY_COMPONENT_METADATA: Record<string, AnyComponentDef> = {
  'Button': {
...
const DEFAULT_VISIBLE_PROPERTIES: MetadataProperty[] = [
  { name: 'Width', type: 'Number' },
  { name: 'Height', type: 'Number' },
  { name: 'Visible', type: 'Boolean' },
  { name: 'Enabled', type: 'Boolean' }
...
const DEFAULT_NON_VISIBLE_PROPERTIES: MetadataProperty[] = [
  { name: 'Enabled', type: 'Boolean' }
];

const DEFAULT_LAYOUT_PROPERTIES: MetadataProperty[] = [
  { name: 'Width', type: 'Number' },
  { name: 'Height', type: 'Number' },
  { name: 'Visible', type: 'Boolean' },
  { name: 'AlignHorizontal', type: 'Number', options: ['Left', 'Center', 'Right'] },
...
const TYPE_COERCION_PATTERNS: TypeCoercionPattern[] = [
...
const coerceType = (propName: string, currentType: string | undefined): string => {
...
const resolveInheritedData = (componentType: string): ComponentDef => {
  const gen = GENERATED_COMPONENT_METADATA;
  const visited = new Set<string>();
  const allEvents: MetadataEvent[] = [];
  const allMethods: MetadataMethod[] = [];
  const allProperties: MetadataProperty[] = [];

  const collect = (typeName: string | undefined) => {
...
const mergeNamedItems = <T extends { name: string }>(current: T[] = [] as T[], generated: T[] = [] as T[]): T[] => {
  ...
  generated.forEach((item) => {
    ...
    const itemOpts = (itemAny.options as unknown[] | undefined)?.length ?? 0;
    const existingOpts = (existingAny.options as unknown[] | undefined)?.length ?? 0;
    if (itemOpts > existingOpts) {
      map.set(item.name, { ...existing, ...item });
    } else if ((!existingAny.type || existingAny.type === 'Any') && itemAny.type && itemAny.type !== 'Any') {
      map.set(item.name, { ...existing, type: itemAny.type } as T);
    } else {
      const itemParams = (itemAny.parameters as unknown[] | undefined)?.length ?? 0;
      const existingParams = (existingAny.parameters as unknown[] | undefined)?.length ?? 0;
      if (itemParams > existingParams) {
        map.set(item.name, { ...existing, parameters: itemAny.parameters } as T);
      }
    }
  });
...
const ensureBaseProperties = (properties: MetadataProperty[], isVisible: boolean): MetadataProperty[] => {
  const required: MetadataProperty[] = isVisible
    ? [
      { name: 'Width', type: 'Number' },
      { name: 'Height', type: 'Number' },
      { name: 'Visible', type: 'Boolean' }
  ...
  const map = new Map<string, MetadataProperty>(properties.map((p) => [p.name, p]));
  required.forEach((p) => {
...
const inferFallbackProperties = (componentType: string, isVisible: boolean): MetadataProperty[] => {
...
const ensurePaletteCoverage = () => {
  ...
  PALETTE_ENHANCED.forEach((component: { type: string; visible: boolean }) => {
