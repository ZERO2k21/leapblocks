...
function generateComponentCss(comp) {
  ...
  if (props.Width !== void 0 && props.Width !== null) {
  ...
  if (props.TextAlignment || type === "Label") {
    ...
    const resolved = ta === 2 || ta === "center" || ta === "Center" ? "center" : ta === 3 || ta === "right" || ta === "Right" ? "right" : "center";
    styles.textAlign = resolved;
  }
...
function generateIndexHtml(appState) {
  console.log('[HTML] generateIndexHtml called');
  const { appName, screens } = appState;
  console.log('[HTML] appName:', appName, 'screens:', (screens || []).length);
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
function generateStylesCss(appState) {
  console.log('[CSS-OUTPUT] generateStylesCss called');
  console.log('[CSS-OUTPUT] designViewport:', JSON.parse(JSON.stringify(getDesignViewport(appState))));
  ...
  // Log all component CSS being generated
  for (const screen of screens) {
    const comps = [...(screen.components || []), ...(screen.nonVisibleComponents || [])];
    for (const comp of comps) {
      const css = generateComponentCss(comp);
      if (css.trim()) {
        console.log(`[CSS-OUTPUT] ${comp.id} (${comp.type}) CSS:\n${css}`);
      }
    }
  }
...
function generateAppJs(appState) {
  console.log('[APPJS] generateAppJs called');
  ...
  // Log screen structure
  for (const screen of screens) {
    console.log(`[APPJS] Screen "${screen.id}" components: ${(screen.components || []).length} nonVisible: ${(screen.nonVisibleComponents || []).length}`);
    for (const comp of (screen.components || [])) {
      console.log(`[APPJS]   visible: ${comp.id} (${comp.type}) children=${(comp.children || []).length}`);
    }
    for (const comp of (screen.nonVisibleComponents || [])) {
      console.log(`[APPJS]   nonVisible: ${comp.id} (${comp.type})`);
    }
  }
...
function generateWebApp(appState) {
  console.log('[WEBAPP] generateWebApp called with screens:', appState.screens?.map((s) => ({ id: s.id, alignHorizontal: s.alignHorizontal, alignVertical: s.alignVertical })));
  ...
  for (const screen of (appState.screens || [])) {
    ...
    function logComp(comp, indent) {
      ...
      if (comp.children) comp.children.forEach((c) => logComp(c, indent + '  '));
    }
    (screen.components || []).forEach((c) => logComp(c, '  '));
  }
  console.log('='.repeat(80));
...
var ApkBuilder = class {
  ...
  async build(appState, onProgress) {
    ...
    try {
      ...
      console.log("[APK-BUILDER] Calling generateWebApp...");
      const webApp = generateWebApp(normalizedAppState);
      console.log("[APK-BUILDER] generateWebApp completed. Files:", Object.keys(webApp));
      console.log("[APK-BUILDER] app.js length:", webApp["app.js"]?.length);
      console.log("[APK-BUILDER] styles.css length:", webApp["styles.css"]?.length);

      // Log a snippet of app.js around Label creation
      const appJs = webApp["app.js"] || "";
      const labelLines = appJs.split('\n').filter(l => l.includes('.style.display') || l.includes('.style.textAlign') || l.includes('.style.alignItems') || l.includes('.style.justifyContent'));
      console.log("[APK-BUILDER] app.js display/textAlign/alignItems/justifyContent lines:");
      labelLines.forEach(l => console.log("[APK-BUILDER]   " + l.trim()));
