/**
 * CreateProjectPage — Premium project creation with type selector grid.
 */
...
const PROJECT_TYPES = [
    ...
    { id: 'hand-pose-classifier', label: 'Hand Pose', emoji: '🖐️', gradient: 'from-purple-400 to-violet-500', bg: 'bg-purple-50', border: 'border-purple-200', desc: 'Detect hand gestures' },
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                                    {PROJECT_TYPES.map((type, i) => (
                                        ...
                                        >
                                            <div className={`w-14 h-10 rounded-lg flex items-center justify-center text-2xl transition-transform group-hover:scale-110 ${
                                                selectedType?.id === type.id ? 'bg-white shadow-sm' : 'bg-gray-100'
                                            }`}>
                                                {type.emoji}
                                            </div>
                                            <div>
                                                <span className={`text-xs font-bold block ${selectedType?.id === type.id ? 'text-gray-800' : 'text-gray-500'}`}>
                                                    {type.label}
                                                </span>
