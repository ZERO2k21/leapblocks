...
import type { ClassifierRouterProps, ProjectType } from '../types';

export default function ClassifierRouter({
  project,
  onBack,
  onProjectUpdate,
}: ClassifierRouterProps): React.JSX.Element {
  const props = { project, onBack, onProjectUpdate };

  switch (project.type as ProjectType) {
    case 'Image Classifier':
      return <ImageClassifier {...props} />;
    case 'Object Detection':
      return <ObjectDetection {...props} />;
    case 'Pose Classifier':
      return <PoseClassifier {...props} />;
    case 'Hand Pose Classifier':
      return <HandPoseClassifier {...props} />;
    case 'Audio Classifier':
      return <AudioClassifier {...props} />;
    case 'Text Classifier':
      return <TextClassifier {...props} />;
    case 'Numbers (C/R)':
      return <NumbersClassifier {...props} />;
    default:
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            ...
            <button
              onClick={onBack}
              className="px-6 py-2 bg-purple-700 text-white rounded-lg text-sm font-semibold"
            >
              Go Back
            </button>
          </div>
