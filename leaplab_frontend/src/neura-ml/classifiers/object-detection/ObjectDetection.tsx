/**
 * ObjectDetection — COCO-SSD real-time object detection via webcam.
 */
      ...
      <div className="max-w-3xl mx-auto space-y-5 p-6">
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 animate-neura-slide-up">
          ...
          <p className="text-xs text-green-700 leading-relaxed">
            Detects 80+ common objects (person, car, cat, chair…) using a
            pre-trained model. No training needed.
          </p>
        </div>

        {!modelReady && (
          <div className="neura-card p-8 text-center animate-neura-scale">
            <div className="text-5xl mb-4">🔍</div>
            <p className="text-sm text-gray-500 mb-5">
              Load the COCO-SSD model (~10 MB) to start detecting objects.
            </p>
            <button
              onClick={loadModel}
              disabled={loading}
              className="neura-btn-primary px-8 py-3"
            >
              {loading && (
        ...
        {modelReady && (
          <>
            <div className="flex gap-3 items-center animate-neura-slide-up neura-delay-1">
              <button
                onClick={running ? stopDetection : startDetection}
                className={`neura-btn-primary ${
            ...
            <div
              className="relative bg-black rounded-2xl overflow-hidden ring-1 ring-gray-200 animate-neura-scale neura-delay-2"
              style={{ minHeight: 320 }}
            >
              <video
                ref={videoRef}
              ...
              <div className="neura-card p-4 animate-neura-slide-up">
                ...
                <div className="flex flex-wrap gap-2">
                  {detections.map((d: Detection, i: number) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 bg-violet-50 border border-violet-200 rounded-full px-3 py-1.5 animate-neura-bounce"
