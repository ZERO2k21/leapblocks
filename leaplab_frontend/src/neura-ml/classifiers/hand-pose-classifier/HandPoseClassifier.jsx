...
const COLORS = ['neura-color-2', 'neura-color-3', 'neura-color-1', 'neura-color-4'];

export default function HandPoseClassifier({ project, onBack }) {
    ...
    const startWebcam = async (classId) => {
        try {
    ...
    const captureGesture = () => {
        ...
        const landmarks = Array.from({ length: 21 }, () => [Math.random(), Math.random(), Math.random()]);
        ...
        <ClassifierLayout project={project} onBack={onBack}>
            <div className="neura-panels">
                ...
                <div className="neura-panel neura-panel-left">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => { setClasses((p) => [...p, { id: nextId, name: `Gesture ${nextId}`, samples: [] }]); setNextId((n) => n + 1); }}
                            className="neura-btn-ghost text-violet-600">+ Add Class</button>
                    </div>
                            ...
                            <div key={cls.id} className="neura-class-card animate-neura-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
                                ...
                                <div className="p-4 flex gap-3 items-start">
                                    <div className="shrink-0">
                                        <button
                                            onClick={() => capturing === cls.id ? stopWebcam() : startWebcam(cls.id)}
                                        ...
                                        {capturing === cls.id && (
                                            <button onClick={captureGesture} className="mt-2 w-full py-2 bg-violet-500 text-white text-xs rounded-lg font-bold hover:bg-violet-600 transition-colors shadow-sm">
                                                Capture
                                            </button>
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[11px] font-bold text-gray-400 mb-2 uppercase tracking-wider">
                                            {cls.samples.length} gesture{cls.samples.length !== 1 ? 's' : ''}
                                        </p>
                                        {cls.samples.length > 0 ? (
                                            <div className="flex flex-wrap gap-1.5">
                                                {Array.from({ length: Math.min(cls.samples.length, 8) }).map((_, idx) => (
                                                    <div key={idx} className="w-9 h-9 bg-violet-50 border border-violet-100 rounded-lg flex items-center justify-center text-sm animate-neura-bounce" style={{ animationDelay: `${idx * 0.03}s` }}>
                                                        🤚
                                                    </div>
                                                ))}
                                            </div>
                ...
                <div className="neura-panel neura-panel-center">
                    <TrainingPanel status={status} progress={progress} accuracy={0.91} canTrain={canTrain} onTrain={handleTrain} trained={trained}
                    ...
                    <div className="neura-side-panel">
                        ...
                        {!trained ? (
                            <div className="p-6 text-center">
                                ...
                                <p className="text-[11px] text-gray-400 leading-relaxed">Train your hand pose model first.</p>
                            </div>
                        ) : (
                            <div className="p-4">
                                <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-center animate-neura-bounce">
                                    <div className="text-green-700 text-sm font-bold">✓ Model Ready</div>
                                    <p className="text-green-600 text-xs mt-1">Show hand gestures to the camera</p>
                                </div>
                            </div>
                        )}
                    </div>
