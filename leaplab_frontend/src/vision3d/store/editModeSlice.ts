...
interface EditModeSliceState {
  editMode: string
  editShapeId: string | null
  selectedVertices: VertexRef[]
  selectedEdges: EdgeRef[]
  selectedFaces: FaceRef[]
  editTool: string | null
  geometryCache: Record<string, THREE.BufferGeometry>
  geometryVersion: number
  proportionalRadius: number
}

interface EditModeSliceActions {
  setEditMode: (mode: string) => void
  setEditTool: (tool: string | null) => void
  setProportionalRadius: (radius: number) => void
  selectVertex: (shapeId: string, index: number, multi?: boolean) => void
  selectEdge: (shapeId: string, a: number, b: number, multi?: boolean) => void
  selectEdges: (edges: EdgeRef[], append?: boolean) => void
  selectFace: (shapeId: string, index: number, multi?: boolean) => void
  clearComponentSelection: () => void
  cacheGeometry: (shapeId: string, geometry: THREE.BufferGeometry) => void
  removeCachedGeometry: (shapeId: string) => void
  applyGeometryEdit: (shapeId: string, newGeometry: THREE.BufferGeometry) => void
}

export type EditModeSlice = EditModeSliceState & EditModeSliceActions

interface StoreState extends EditModeSliceState {
  selectedIds: string[]
  shapes: ShapeRef[]
  pushHistory: () => void
}

export const createEditModeSlice: StateCreator<StoreState, [], [], EditModeSlice> = (set, get) => ({
  editMode: 'object',
  editShapeId: null,
  selectedVertices: [],
  selectedEdges: [],
  ...
  selectVertex: (shapeId, index, multi = false) => {
    set((state) => {
      if (multi) {
        const exists = state.selectedVertices.find(
          (v) => v.shapeId === shapeId && v.index === index
        )
        return {
          selectedVertices: exists
            ? state.selectedVertices.filter(
                (v) => !(v.shapeId === shapeId && v.index === index)
              )
  ...
  selectEdge: (shapeId, a, b, multi = false) => {
    set((state) => {
      const key = (e: EdgeRef) =>
        `${e.shapeId}:${Math.min(e.a, e.b)}-${Math.max(e.a, e.b)}`
  ...
  selectFace: (shapeId, index, multi = false) => {
    set((state) => {
      if (multi) {
        const exists = state.selectedFaces.find(
          (f) => f.shapeId === shapeId && f.index === index
        )
        return {
          selectedFaces: exists
            ? state.selectedFaces.filter(
                (f) => !(f.shapeId === shapeId && f.index === index)
              )
