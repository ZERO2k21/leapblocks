...
export function getR3FRefs(): R3FRefs {
  return window.__r3fRefs || { camera: null, gl: null, scene: null }
}

export function getShapeMeshes(): THREE.Mesh[] {
  const { scene } = getR3FRefs()
  const meshes: THREE.Mesh[] = []
  scene?.traverse?.((child) => {
    if ((child as THREE.Mesh).isMesh && (child as THREE.Mesh).userData.shapeId && !(child as THREE.Mesh).userData.gizmoAxis) {
...
export function getComponentPositions(): ComponentPosition[] | null {
  const state = use3DStore.getState() as any
  const { editMode, editShapeId, geometryCache } = state
  if (editMode === 'object' || !editShapeId) return null
  const geo = geometryCache[editShapeId] as THREE.BufferGeometry | undefined
  if (!geo) return null
  ...
  const mesh: THREE.Mesh | null = (() => {
    ...
    scene?.traverse?.((child) => {
      if (found) return
      if ((child as THREE.Mesh).isMesh && (child as THREE.Mesh).userData.shapeId === editShapeId) found = child as THREE.Mesh
    })
    return found
  })()
  if (!mesh) return null
  ...
  const results: ComponentPosition[] = []
...
export function worldToScreen(worldPos: THREE.Vector3): { x: number; y: number } {
...
export function isInsideSelection(screenX: number, screenY: number, start: Point | null, current: Point | null, lasso: Point[] | null): boolean {
...
export function applySelection(selectedItems: SelectionItem[], additive: boolean): void {
  const store = use3DStore.getState() as any

  if (store.editMode === 'object') {
    const shapeIds = selectedItems.map(item => item.shapeId)
    if (additive) {
      const currentIds = new Set<string>(store.selectedIds)
    ...
    if (mode === 'vertex') {
      ...
      if (additive) {
        const existing = new Set((store.selectedVertices as Array<{ index: number }>).map(v => v.index))
      ...
      if (additive) {
        const edgeKey = (e: { a: number; b: number }) => `${Math.min(e.a, e.b)}-${Math.max(e.a, e.b)}`
        const existing = new Set((store.selectedEdges as Array<{ a: number; b: number }>).map(edgeKey))
      ...
      if (additive) {
        const existing = new Set((store.selectedFaces as Array<{ index: number }>).map(f => f.index))
...
export function selectInRectangle(start: Point, current: Point, additive: boolean): void {
  const store = use3DStore.getState() as any
  ...
  if (store.editMode === 'object') {
    ...
    for (const mesh of meshes) {
      ...
      if (isInsideSelection(screen.x, screen.y, start, current, null)) {
        selected.push({ shapeId: mesh.userData.shapeId as string })
      }
    }
...
export function selectInCircle(screenX: number, screenY: number, additive: boolean): void {
  const store = use3DStore.getState() as any
...
export function selectInLasso(points: Point[], additive: boolean): void {
