...
export function getDirectSelection(
...
export function falloffSmooth(r: number): number {
  if (r >= 1) return 0
  return 3 * r * r - 2 * r * r * r
}

export function applyProportionalFalloff(r: number): number {
...
export function getConnectedVertices(
  ...
  selectedEdges: EdgeSelection[],
  selectedFaces: ComponentSelection[]
): { selected: Set<number>; neighbors: Set<number> } {
  const connected = new Set<number>()
  const idx = geo.index
  const posAttr = geo.attributes.position
