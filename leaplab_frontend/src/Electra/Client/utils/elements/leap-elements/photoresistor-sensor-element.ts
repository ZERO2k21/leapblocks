 ...
 *   value     — light level in lux (0 … 1000)
 *   threshold — DO goes LOW when lux < threshold (0 … 1000, default 500)
 *   ledPower  — power LED on (always true when VCC connected)
 *   ledDO     — DO LED on when DO is LOW (light below threshold)
 *
 * Voltage mapping (voltage-divider, LDR + 10kΩ series, VCC=5V):
 *   R_ldr ≈ 500000 / lux  (empirical: ~500kΩ in dark, ~500Ω in bright)
...
export class PhotoresistorSensorElement extends LitElement {
  ...
  /** DO threshold: DO pin goes LOW when lux < threshold */
  @property({ type: Number }) threshold = 500;

  /** Power LED state */
  @property({ type: Boolean }) ledPower = false;

  /** DO LED state (mirrors DO pin — ON when light < threshold) */
  ...
  render() {
    ...
    const ldrFill = `rgb(${218 + ldrBrightness > 255 ? 255 : 218 + ldrBrightness}, ${227 + ldrBrightness > 255 ? 255 : 227 + ldrBrightness}, ${235 + ldrBrightness > 255 ? 255 : 235 + ldrBrightness})`;
    ...
    return html`
      <div class="wrap">
        ...
        >
          ...
          <!-- Photo sensor body -->
          <rect x="18.9" y="20.1" width="11.2" height="22.2" fill="none" stroke="#fff" stroke-width="1.08px" />

          <!-- LDR circles — brightness-reactive fill -->
          <circle cx="24.5" cy="25.6" r="3.14" fill="${ldrFill}" />
          <circle cx="24.5" cy="36.8" r="3.14" fill="${ldrFill}" />
          ...
          <filter id="ldrLedFilter" x="-0.8" y="-0.8" height="5.2" width="5.8">
            <feGaussianBlur stdDeviation="2" />
          </filter>
          ${showPower ? svg`<circle cx="124.5" cy="7" r="4" fill="green" filter="url(#ldrLedFilter)" />` : ''}

          <rect x="118" y="52.6" width="13" height="4.29" fill="#dae3eb" />
          <rect x="121" y="52.5" width="6.55" height="4.59" fill="#fffefe" />
          ${showDO ? svg`<circle cx="124.5" cy="55" r="4" fill="red" filter="url(#ldrLedFilter)" />` : ''}
