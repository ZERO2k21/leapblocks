...
export class RotaryDialerElement extends LitElement {
  ...
  private addDialerAnim(digit: Digit) {
    requestAnimationFrame(() => {
      this.isDialing = true;
      // Fire dial-start immediately when the user selects a digit
      this.dispatchEvent(
        new CustomEvent('dial-start', {
          bubbles: true,
          composed: true,
          detail: { digit: this.digit },
        })
      );
      // Rotate the slots to the finger-stop position
    ...
    >
      ...
      <g transform="translate(1 1)">
        ...
        <path
          ...
          @transitionstart="${() => {
            // No-op — the real start is handled by addDialerAnim
          }}"
          @transitionend="${() => {
            if (this.isDialing) {
              // Dial reached the finger stop. Fire dial-end and start return.
              this.dispatchEvent(
                new CustomEvent('dial-end', {
                  bubbles: true,
                  composed: true,
                  detail: { digit: this.digit },
                })
              ...
              // Brief pause at the finger stop, then spring back home
              setTimeout(() => this.removeDialerAnim(), 150);
            }
          }}"
      ...
      <circle id="container" fill-opacity=".5" fill="#070707" cx="132.5" cy="132.5" r="132.5" pointer-events="none" />
      <g class="text" font-family="Marker Felt, monospace" font-size="21" fill="#FFF">
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(0); }} x="129" y="243">0</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(9); }} x="78" y="230">9</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(8); }} x="40" y="194">8</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(7); }} x="28" y="145">7</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(6); }} x="35" y="97">6</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(5); }} x="72" y="58">5</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(4); }} x="117" y="41">4</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(3); }} x="168" y="47">3</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(2); }} x="210" y="79">2</text>
        <text @mousedown=${(e: Event) => { e.stopPropagation(); this.dial(1); }} x="230" y="126">1</text>
      </g>
      <path
