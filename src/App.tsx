    ...
    const handleWorkspaceChange = useCallback((event: Blockly.Events.Abstract) => {
        ...
        try {
            if (editorMode === 'upload') {
                ...
                const formattedCode = `// LeetBlocks - Arduino Code\n\n${code || 'void setup() {\n  // Setup code here\n}\n\nvoid loop() {\n  // Loop code here\n}'}`;
        ...
        if (appMode === 'blocks' && blocklyDiv.current) {
            ...
            const timer = setTimeout(() => {
                if (blocklyDiv.current) {
                    ...
                    workspaceRef.current.registerToolboxCategoryCallback('LEET_VARIABLES', (ws) => {
                        ...
                        // 2. Variable Getters
                        const variables = ws.getVariablesOfType('');
                        if (variables.length > 0) {
                            variables.sort((a: any, b: any) => a.name.localeCompare(b.name));
                            variables.forEach(v => {
                                ...
                                field.textContent = (v as any).name;
                                block.appendChild(field);
                                xmlList.push(block);
                            });
                        }

                        // 3. Setter/Changer/Show/Hide
                        const blockTypes = [
                            'data_setvariableto',
                        ...
                        const defaultVar = variables.length > 0 ? variables[0] : null;
                        ...
                        blockTypes.forEach(type => {
                            ...
                            if (defaultVar) {
                                ...
                                field.textContent = (defaultVar as any).name;
                                block.appendChild(field);
                            }
            ...
            {promptState.isOpen && (
                <div style={styles.modalOverlay}>
                    <div style={styles.modalContent}>
                        <h3 style={styles.modalTitle}>{promptState.message}</h3>
                        <input
