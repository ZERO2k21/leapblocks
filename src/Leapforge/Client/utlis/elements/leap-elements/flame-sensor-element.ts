 ...
 * 4-pin: VCC · GND · DOUT (digital) · AOUT (analog)
 *
 * Simulation properties:
 *   value     — flame intensity 0–100 (0 = no flame, 100 = max flame)
 *   threshold — DOUT goes LOW when intensity > threshold (0–100, default 50)
 *   ledPower  — power LED (always on when VCC connected)
 *   ledSignal — signal LED (on when flame detected, i.e. DOUT LOW)
 *
 * Voltage mapping (IR phototransistor + 10kΩ pull-up, VCC=5V):
 *   V_aout = VCC × (1 - intensity/100)   (high voltage = no flame, low = flame)
...
export class FlameSensorElement extends LitElement {
  ...
  /** DOUT threshold: DOUT goes LOW when intensity > threshold */
  @property({ type: Number }) threshold = 50;

  /** Power LED — on when VCC is connected */
  @property({ type: Boolean }) ledPower = false;

  /** Signal LED — on when flame detected (DOUT LOW) */
  ...
  readonly pinInfo: ElementPin[] = [
    ...
    { name: 'AOUT', y: 43.7, x: 199, number: 4, signals: [analog(0)] },
  ];
  ...
  /** AOUT voltage: high when no flame, low when flame detected */
  private _intensityToVoltage(intensity: number): number {
    const i = Math.max(0, Math.min(100, intensity));
    return 5.0 * (1 - i / 100);
  }

  render() {
    const intensity  = Number(this.value) || 0;
    const threshold  = Number(this.threshold) || 50;
    const flameOn    = intensity > threshold;          // flame detected
    const voltage    = this._intensityToVoltage(intensity);
    const adcRaw     = Math.round((voltage / 5.0) * 1023);

    // Visual: flame glow on the IR sensor when flame is detected
    const glowAlpha  = (intensity / 100).toFixed(2);
    const showPower  = this.ledPower;
    const showSignal = this.ledSignal || flameOn;

    // IR sensor fill — redder when flame detected
    const irFill = flameOn
      ? `rgba(255,${Math.round(100 - intensity)},0,0.9)`
      : '#29261c';
    ...
    return html`
      <div class="wrap">
        ...
        >
          ...
          <!-- IR sensor body -->
          <rect x="45.5" y="20.1" width="11.2" height="22.2" fill="none" stroke="#fff" stroke-width="1.08px" />
          <circle cx="51.1" cy="25.6" r="3.14" fill="${irFill}" />
          <circle cx="51.1" cy="36.8" r="3.14" fill="${irFill}" />

          <!-- Flame glow on IR sensor -->
          ${flameOn ? svg`
            <circle cx="51.1" cy="31.2" r="${8 + intensity * 0.08}" fill="rgba(255,120,0,${glowAlpha})" opacity="0.5">
              <animate attributeName="r" values="${8 + intensity * 0.08};${10 + intensity * 0.1};${8 + intensity * 0.08}" dur="0.6s" repeatCount="indefinite"/>
            </circle>
          ` : ''}

          <!-- IR leads -->
          <path d="m51.1 25.7c0-0.199-0.079-0.39-0.219-0.53-0.141-0.141-0.332-0.22-0.53-0.22h-13.2v1.5h13.2c0.198 0 0.389-0.079 0.53-0.219 0.14-0.141 0.219-0.332 0.219-0.53z" fill="#a8b6ba" />
          <path d="m51.1 36.7c0-0.198-0.079-0.389-0.219-0.53-0.141-0.14-0.332-0.219-0.53-0.219h-13.2v1.5h13.2c0.198 0 0.389-0.079 0.53-0.22 0.14-0.14 0.219-0.331 0.219-0.53z" fill="#a8b6ba" />
          <path d="m35.2 20.1h-25.2c-5.49 0-9.94 4.45-9.94 9.94v1e-3c0 5.49 4.45 9.94 9.94 9.94h25.2z" fill="#29261c" />

          <clipPath id="flame-clip-c">
            <path d="m35.18 20.14h-25.2c-5.49 0-9.94 4.45-9.94 9.94v1e-3c0 5.49 4.45 9.94 9.94 9.94h25.2z" />
          </clipPath>
          <g clip-path="url(#flame-clip-c)">
            <path d="m37.68 21.94c-12.6 0-25.1-0.227-37.7 0.917-0.196 0.018 0 4.25 0 4.25 12.8-1.41 25-1.08 37.7-0.669z" fill="#47434d" />
          </g>
          <rect x="32.6" y="17.9" width="5.06" height="25.7" fill="#29261c" />
          <clipPath id="flame-clip-b">
            <rect x="32.68" y="17.94" width="5.06" height="25.7" />
          </clipPath>
          <g clip-path="url(#flame-clip-b)">
            <path d="m51.98 20.84c-12.6 0-25.1-0.228-37.7 0.916-0.195 0.018 0 4.25 0 4.25 12.8-1.41 25-1.08 37.7-0.669z" fill="#47434d" />
          </g>
          ...
          <filter id="flameLedFilter" x="-0.8" y="-0.8" height="5.2" width="5.8">
            <feGaussianBlur stdDeviation="2" />
          </filter>
          ${showPower ? svg`<circle cx="153.2" cy="7.5" r="7" fill="#80ff80" filter="url(#flameLedFilter)" />` : ''}

          <rect x="145" y="48.3" width="16.7" height="5.52" fill="#dae3eb" />
          <rect x="149" y="48.1" width="8.43" height="5.9" fill="#fffefe" />
          ${showSignal ? svg`<circle cx="153.2" cy="51.3" r="7" fill="#ff6600" filter="url(#flameLedFilter)" />` : ''}
          ...
          <clipPath id="flame-clip-a">
            <circle cx="105.08" cy="16.04" r="6.56" />
          </clipPath>
          <g clip-path="url(#flame-clip-a)" fill="none" stroke="#3f3c40" stroke-width="2.5px">
            <path d="m105.08 6.74v18.6" />
            <path d="m115.08 16.04h-18.6" />
          </g>

          <path d="m149 19.8c5.5 0 9.96 4.46 9.96 9.96s-4.46 9.96-9.96 9.96-9.96-4.46-9.96-9.96 4.46-9.96 9.96-9.96zm0 2.4c4.17 0 7.55 3.38 7.55 7.55 0 4.17-3.38 7.55-7.55 7.55s-7.55-3.38-7.55-7.55c0-4.17 3.38-7.55 7.55-7.55z" fill="#d4d0d1" />

          <!-- Pin header -->
          <path d="m169.1 10.2v38.6h8.39v-38.6z" fill="#1c2546" stroke="#fff" stroke-linejoin="round" stroke-width=".9px" />
          <g fill="#29261c">
          ...
          <text font-family="monospace" font-size="3.8px" font-weight="bold" text-anchor="middle">
            <tspan x="105" y="52" fill="${flameOn ? '#f97316' : '#64748b'}">${flameOn ? '🔥 FLAME' : 'NO FLAME'}</tspan>
            <tspan x="105" y="57" fill="#bef264">${voltage.toFixed(2)}V / ${adcRaw}</tspan>
          </text>
        </svg>
      </div>
    `;
