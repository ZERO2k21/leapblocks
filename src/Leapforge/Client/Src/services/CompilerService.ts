...
function clientSideTranspile(code: string): TranspileResult {
  try {
    ...
    // Remove class-type variable declarations (e.g. Adafruit_SSD1306 oled(...); or MyClass obj;)
    // These are constructor calls / object instantiations that have no JS equivalent after #includes are stripped
    js = js.replace(/^\s*[A-Z][A-Za-z0-9_]*\s+\w+\s*\([^)]*\)\s*;/gm, '// [removed class instantiation]');
    js = js.replace(/^\s*[A-Z][A-Za-z0-9_]*\s+\w+\s*;/gm, '// [removed class variable]');
    // #define → const
...
const Adafruit_SSD1306 = class { constructor(){} begin(){return true;} clearDisplay(){} display(){} setTextSize(){} setTextColor(){} setCursor(){} print(){} println(){} drawPixel(){} fillRect(){} drawRect(){} drawCircle(){} fillCircle(){} setRotation(){} invertDisplay(){} startscrollright(){} stopscroll(){} };
const Adafruit_GFX = class { constructor(){} };
const LiquidCrystal_I2C = class { constructor(){} begin(){} print(){} println(){} setCursor(){} clear(){} backlight(){} noBacklight(){} };
const LiquidCrystal = class { constructor(){} begin(){} print(){} println(){} setCursor(){} clear(){} };
const Servo = class { constructor(){this._angle=90;} attach(){} write(a){this._angle=a;} read(){return this._angle;} detach(){} };
const DHT = class { constructor(){} begin(){} readTemperature(){return 25.0;} readHumidity(){return 50.0;} };
const IRrecv = class { constructor(){} enableIRIn(){} decode(){return false;} resume(){} };
const decode_results = class { constructor(){} };
const SoftwareSerial = class { constructor(){} begin(){} print(){} println(){} available(){return 0;} read(){return -1;} };
const Stepper = class { constructor(){} setSpeed(){} step(){} };
const MFRC522 = class { constructor(){} PCD_Init(){} PICC_IsNewCardPresent(){return false;} PICC_ReadCardSerial(){return false;} };
const Keypad = class { constructor(){} getKey(){return null;} };
const makeKeymap = (k) => k;
const U8g2_SSD1306_128X64_NONAME_F_HW_I2C = class { constructor(){} begin(){} clearBuffer(){} sendBuffer(){} setFont(){} drawStr(){} setCursor(){} print(){} println(){} };
${js}
