...
var Keypad = (typeof Keypad !== 'undefined' && Keypad) || class {constructor(_keymap, _rowPins, _colPins, _rows, _cols) {this._keymap = _keymap || [];this._rowPins = _rowPins || [];this._colPins = _colPins || [];this._rows = _rows || 4;this._cols = _cols || 4;this._pressedKey = null;}getKey() {const k = this._pressedKey;this._pressedKey = null;return k;}isPressed(key) { return this._pressedKey === key; }getState() { return 0; }addEventListener() {}_simulatePress(key) { this._pressedKey = key; }};
...
var WiFiClient = (typeof WiFiClient !== 'undefined' && WiFiClient) || class { constructor(){ this._connected=false; this._buffer=''; } connect(h,p){ console.warn('[WiFiClient] Not available on this board'); return false; } connected(){ return this._connected; } stop(){ this._connected=false; this._buffer=''; } print(d){ this._buffer+=String(d); } println(d){ this._buffer+=String(d)+'\\n'; } available(){ return this._buffer.length; } read(){ if(this._buffer.length===0)return -1; const c=this._buffer.charCodeAt(0); this._buffer=this._buffer.substring(1); return c; } readString(){ const s=this._buffer; this._buffer=''; return s; } };
var HTTPClient = (typeof HTTPClient !== 'undefined' && HTTPClient) || class { constructor(){ this._url=''; this._headers=new Map(); this._code=0; this._body=''; this._timeout=5000; } begin(url){ console.warn('[HTTPClient] Not available on this board'); this._url=url; return false; } addHeader(n,v){ this._headers.set(n,v); } setTimeout(t){ this._timeout=t; } async GET(){ console.warn('[HTTPClient] Not available on this board'); return -2; } async POST(p){ console.warn('[HTTPClient] Not available on this board'); return -2; } async PUT(p){ console.warn('[HTTPClient] Not available on this board'); return -2; } async DELETE(){ console.warn('[HTTPClient] Not available on this board'); return -2; } async PATCH(p){ console.warn('[HTTPClient] Not available on this board'); return -2; } getString(){ return ''; } getSize(){ return 0; } end(){} };
