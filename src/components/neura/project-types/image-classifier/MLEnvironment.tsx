                    ...
                    {trained && (
                        <div style={{ background: "#13131f", border: "1px solid #1e1e2e", borderRadius: 16, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
                            ...
                            {[
                                ...
                                { label: "Copy embed code", desc: "Use in your app", action: () => navigator.clipboard.writeText(`<!-- Neura ML Model -->\n<script>const model = ${JSON.stringify({ classes: classes.map(c => c.name) })}</script>`).then(() => alert("Copied!")) },
