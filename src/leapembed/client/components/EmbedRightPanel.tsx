/**
 * EmbedRightPanel.tsx
 * Right-side panel: Stage canvas (stage mode), SpritePanel,
 * code preview (upload mode), log/serial tabs.
 */
...
import { PythonEditorTab } from './PythonEditorTab';
...
export const EmbedRightPanel: React.FC<EmbedRightPanelProps> = ({
    ...
    isCameraOn, showGrid, isRunning, compiledScripts, workspaceRef,
    ...
    generatedCode, logMessages, activeTab, onTabChange,
    serialMessages, baudRate, setBaudRate, lineEnding, setLineEnding,
    isConnected, onSendSerial, onUpload, addLog,
    ...
    const CANVAS_WIDTH = STAGE_CONFIG.WIDTH;
    const CANVAS_HEIGHT = STAGE_CONFIG.HEIGHT;
    const fsScale = fullscreenScale;
    ...
    return (
        ...
        >
            {/* ── Stage Container (stage mode only) ─────────────────────── */}
            {(editorMode === 'stage' || isFullscreen) && (
                ...
                >
                    ...
                    <div style={{
                        position: 'relative',
                        width: isFullscreen ? `${Math.round(CANVAS_WIDTH * fsScale)}px` : '100%',
                        flexShrink: 0,
                    }}>
                        <Stage
                            sprites={sprites}
                            ...
                            width={isFullscreen ? Math.round(CANVAS_WIDTH * fsScale) : CANVAS_WIDTH}
                            height={isFullscreen ? Math.round(CANVAS_HEIGHT * fsScale) : CANVAS_HEIGHT}
                        />
                        {/* Monitor overlays */}
                        <EmbedMonitors
                            ...
                            onShowVariable={onShowVariable}
                            onHideVariable={onHideVariable}
            ...
            {/* ── Sprite & Stage Panel (hidden in fullscreen) ────────────── */}
            {!isFullscreen && (
                <SpritePanel
                    sprites={sprites}
                    selectedSpriteId={selectedSpriteId}
            ...
            {/* ── Generated Code panel (upload mode only) ───────────────── */}
            {editorMode === 'upload' && !isFullscreen && (
                <div style={{
                    flex: 1, display: 'flex', flexDirection: 'column',
                    minHeight: 0, borderRadius: 8, border: '1px solid #eee', overflow: 'hidden',
                }}>
                    <div style={styles.codeHeader}>
                        <span style={{ fontWeight: 600, fontSize: 13 }}>Generated Code</span>
